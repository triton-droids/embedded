...
- Per-step timing breakdown: robot read, IMU get, obs build, policy inference,
  safety/limits, robot write, robot feedback, status print.
- Rolling window stats (p50/p95/max) printed periodically.
- Overrun counter when step_time > dt * overrun_warn_ratio.
"""
...
class TimingStats:
    """
    Rolling-window timing statistics collector.

    Stores durations (seconds) per named section in a deque (rolling window).
    Can report p50/p95/max in milliseconds and overrun counts.
    """
    window: int = 500
    ...
    def report(self, header: str = "", reset_overrun: bool = False) -> str:
        parts: list[str] = []
        if header:
            parts.append(header)

        parts.append(f"steps={self.step_count} overruns={self.overrun_count}")

        # Print each section's p50/p95/max in ms
        for name, q in self.buf.items():
            arr = list(q)
            if not arr:
                continue
            p50 = self._pct(arr, 50.0)
            p95 = self._pct(arr, 95.0)
            mx = max(arr)
            if p50 is None or p95 is None:
                continue
            parts.append(f"{name}: p50={p50*1000.0:.3f} p95={p95*1000.0:.3f} max={mx*1000.0:.3f} ms")
...
class ImuReader:
    def __init__(self, cfg: dict[str, Any]):
        ...
        # Optional IMU-thread timing stats (does not affect control loop)
        self._timing_enabled = bool(cfg.get("imu_timing_enabled", False))
        self._imu_stats = TimingStats(window=int(cfg.get("imu_timing_window", 300))) if self._timing_enabled else None
        self._last_imu_report = 0.0
        self._imu_report_interval = float(cfg.get("imu_timing_print_interval_s", 2.0))
    ...
    def get(self) -> tuple[np.ndarray, np.ndarray]:
        # Keep this cheap: lock + small copies.
        with self._lock:
    ...
    def _loop(self) -> None:
        try:
            ...
            for sample in gen:
                ...
                # Compute up vector and angular velocity
                ...
                if self._timing_enabled and self._imu_stats is not None:
                    self._imu_stats.add("imu_loop", time.perf_counter() - t0)
                    now = time.time()
                    if (now - self._last_imu_report) >= self._imu_report_interval:
                        self._last_imu_report = now
                        print("[IMU_TIMING] " + self._imu_stats.report(reset_overrun=False))
...
class ObservationBuilder:
    def __init__(
        ...
        if not np.array_equal(
            np.arange(len(self.policy_to_real)),
            np.arange(len(self.policy_to_real))[self.real_to_policy][self.policy_to_real],
        ):
    ...
    def observe(
        ...
        # NOTE: np.roll allocates; it's measurable overhead when frame_stack > 1.
        # If this shows up as a bottleneck, replace with a circular buffer index.
        if self.stack_frame_major:
...
class PolicyRunner:
    def __init__(self, policy_path: Path, device: str):
        ...
        # Use inference_mode (faster/safer for inference) when available
    ...
    def act(self, obs: np.ndarray) -> np.ndarray:
        ...
        if self._use_inference_mode and hasattr(torch, "inference_mode"):
...
class RunPolicyController:
    def __init__(self, cfg: dict[str, Any], policy_path: Path, dry_run: bool = False):
        ...
        # ---- timing config (safe defaults without requiring config keys) ----
        timing_cfg = cfg.get("timing", {}) if isinstance(cfg.get("timing", {}), dict) else {}
        self.enable_timing = bool(timing_cfg.get("enabled", True))
        self.timing = TimingStats(window=int(timing_cfg.get("window", 500)))
        self.timing_interval = float(timing_cfg.get("print_interval_s", 1.0))  # print every N seconds
        self._last_timing_print = 0.0
        self.overrun_warn_ratio = float(timing_cfg.get("overrun_warn_ratio", 1.0))  # count overrun if step > dt*ratio
        self.reset_overrun_each_report = bool(timing_cfg.get("reset_overrun_each_report", False))
    ...
    def run(self, max_steps: int | None = None) -> None:
        ...
        while self.running and self.robot.connected:
            ...
            if self.enable_timing:
                ...
                self.timing.add("step_total", step_dt)
                self.timing.step_count = self.step_idx
                if step_dt > (self.dt * self.overrun_warn_ratio):
                    self.timing.note_overrun()
    ...
    def _control_step(self) -> None:
        ...
        if self.enable_timing:
            self.timing.add("robot_read", t1 - t0)
            self.timing.add("imu_get", t2 - t1)
            self.timing.add("obs_build", t3 - t2)
    ...
    def _maybe_print_timing(self) -> None:
        now = time.time()
        if (now - self._last_timing_print) < self.timing_interval:
            return
        self._last_timing_print = now
        print("[TIMING] " + self.timing.report(reset_overrun=self.reset_overrun_each_report))
